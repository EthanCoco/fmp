	    ...
	    <div class="layui-side-scroll" style="height: 100%;width: auto;">
	      	<div id="bus_designer_content" style="z-index:1000;height: 697px;position: relative;">
				...
				<div class="layui-btn-group" style="float: right;margin-right: 30px;margin-top: -48px;">
				  	<button onclick="print_view3()" class="layui-btn">解析预览</button>
...
function init_upload_file(){
	layui.use(['upload','layer'], function(){
		...
		$.getJSON("<?= yii\helpers\Url::to(['default/getexcelhtml3']); ?>",
		{
			'bus_id':bus_id,
		...
		function(json){
			...
			if(json.file != ""){
		 	...
		 	upload.render({
			    elem: '#file_btn',
			    url: "<?= yii\helpers\Url::to(['default/uploadexcel3']); ?>",
...
/*预览*/
function print_view3(){
	parent.layer.open({
		type:2,
		area:["800px","800px"],
		content:"<?= yii\helpers\Url::to(['default/printview3']); ?>"+"?busName="+bus_id+"&flow_id="+parent.flow_node_id,
	});
}
</script>
</body>
